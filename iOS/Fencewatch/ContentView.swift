...
struct ContentView: View {
    ...
    @State private var editingEntry: RepairEntry?
...
struct EntryFormView: View {
    ...
    @State private var stage: String = RepairEntry.stageOptions.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    Picker("Stage", selection: $stage) {
                        ForEach(RepairEntry.stageOptions, id: \.self) { Text($0).tag($0) }
                    }
            ...
            .navigationTitle(mode.id == "add" ? "New Repair" : "Edit Repair")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = RepairEntry(title: title, stage: stage, date: date, note: note, photoData: photoData)
            store.add(entry)
        case .edit(var entry):
            entry.title = title
            entry.stage = stage
